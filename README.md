...
Markdown ファイルの先頭に YAML front matter で `tags` を記述すると、そのファイル内の全タスクにタグが自動付与されます（本文中の `#tag` 記法と合算されます）。
...
上記の場合、タスクは `#projectA` `#work` `#urgent` の 3 タグとして扱われ、タグ別ビューや CLI の `--tag` フィルタでヒットします。
